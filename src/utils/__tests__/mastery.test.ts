...
describe("generateMasteryTrack", () => {
  test("Short verse (< 15 words) generates exactly 4 steps and follows rules", () => {
    ...
    expect(track[3].mode).toBe("SCRIBE");
  ...
  test("Medium verse (15 - 30 words) generates exactly 7 steps and follows rules", () => {
    const mediumText =
    ...
    expect(track[2].missingCount).toBe(5);
    expect(track[4].missingCount).toBe(6);
  });

  test("Long verse (> 30 words) generates exactly 8 steps and follows rules", () => {
    const longText =
    ...
    expect(track[2].missingCount).toBe(8);
    expect(track[4].missingCount).toBe(8);
    expect(track[6].missingCount).toBe(12);
  });
});
